...
/**
 * When the tip changes: four times a day, at [TIP_SWITCH_HOURS].
 *
 ...
 *   to prevent.
 *
 * ## Why these four hours
 *
 * 06:00, 12:00, 18:00 and 23:00 are not arbitrary: they are [DayPart] boundaries. Each switch
 * therefore hands over a tip drawn from the pool for the part of the day being entered, and the
 * tip on screen always belongs to the hours you are actually in. That alignment is the real win,
 * and it is the thing to protect — if these hours ever change, change [DayPart] with them, or the
 * pools quietly stop matching the clock again.
 *
 * ## The night, and what is left of the cost
 *
 * The list stopped at 18:00 until 2026-08-02, so the evening tip stayed up until 06:00 and both
 * night pools were unreachable except by tapping — 29 tips written for someone awake at 3am, dark
 * on the schedule. Adding `23` closes the half of that which matters: a reader awake at midnight
 * now gets a line written for the hour rather than an evening one that has been up for six hours.
 *
 * **`sleep_early` (00:00-05:59) is still tap-only**, because the fifth [DayPart] boundary is
 * midnight and there is no switch there. That one is deliberate rather than pending: the 23:00
 * tip is written for the same reader in the same state, so a midnight switch would replace a
 * night tip with another night tip at the hour a reader is least likely to be looking. Adding `0`
 * is the entire fix if that judgement ever changes.
 *
 ...
 * catch-up ones.
 */
val TIP_SWITCH_HOURS = listOf(6, 12, 18, 23)
 ...
 * what keeps the small hours on last night's 23:00 tip instead of briefly inventing a fifth slot.
 *
